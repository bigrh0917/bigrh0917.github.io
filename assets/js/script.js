function generateCatalog(articleSelector, dirSelector) {
  ...
  // 在文章元素内获取所有标题元素
  const articleHeadings = article.querySelectorAll('p.h1, p.h2');

  // 遍历文章标题，生成目录
  articleHeadings.forEach(function(heading, index) {
      // alert(heading.className)
      // 获取标题级别
      const headingLevel = heading.className.toLowerCase().replace('h', '');
      // 获取标题文本
      ...
      // 设置不同级别标题的左边距
      let paddingLeft = 5 + (headingLevel - 1) * 10; // 5px起始值，每级标题增加5px内边距
      // 创建目录条目
